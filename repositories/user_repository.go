...
const (
	userTable    = "users"
	selectFields = "id, name, email, password, active, created_at"
	insertFields = "name, email, password, active, created_at"
)
...
func FindUserByEmail(email string) (*models.User, error) {
	...
	err := database.DB.QueryRow(context.Background(), sql, email).Scan(
		&user.ID, &user.Name, &user.Email, &user.Password, &user.Active, &user.CreatedAt,
	)
...
func CreateUser(user models.User) error {
	...
	err = database.DB.QueryRow(context.Background(), sql, user.Name, user.Email, hashedPassword, user.Active, time.Now()).Scan(&user.ID)
	if err != nil {
